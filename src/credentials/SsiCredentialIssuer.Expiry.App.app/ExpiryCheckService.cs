...
public class ExpiryCheckService
{
    ...
    public async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        try
        {
            ...
            {
                await ProcessCredentials(credential, companySsiDetailsRepository, repositories, portalService, stoppingToken);
            }
        }
        catch (Exception ex)
        {
            Environment.ExitCode = 1;
            _logger.LogError("Verified Credential expiry check failed with: {Errors}", ex);
        }
    }
