...
public class ExpiryCheckServiceTests
{
    ...
    public async Task ExecuteAsync_WithInactiveAndEligibleForDeletion_RemovesEntry()
    {
        ...
        {
            new(credentialId, inactiveVcsToDelete.AddDays(-1), null, null, Bpnl, CompanySsiDetailStatusId.INACTIVE, VerifiedCredentialTypeId.DISMANTLER_CERTIFICATE, credentialScheduleData)
        };
    ...
    public async Task ExecuteAsync_WithPendingAndExpiryBeforeNow_DeclinesRequest()
    {
        ...
        var data = new CredentialExpiryData[]
        {
            new(ssiDetail.Id, ssiDetail.ExpiryDate!.Value, ssiDetail.ExpiryCheckTypeId, null, Bpnl, ssiDetail.CompanySsiDetailStatusId, ssiDetail.VerifiedCredentialTypeId, credentialScheduleData)
        };
        A.CallTo(() => _dateTimeProvider.OffsetNow).Returns(now);
        A.CallTo(() => _companySsiDetailsRepository.GetExpiryData(A<DateTimeOffset>._, A<DateTimeOffset>._, A<DateTimeOffset>._))
            .Returns(data.ToAsyncEnumerable());
        ...
        A.CallTo(() => _issuerRepositories.SaveAsync()).MustHaveHappenedOnceExactly();
        A.CallTo(() => _portalService.AddNotification(A<string>._, Bpnl, NotificationTypeId.CREDENTIAL_REJECTED, A<CancellationToken>._)).MustHaveHappenedOnceExactly();
        A.CallTo(() => _portalService.TriggerMail("CredentialRejected", Bpnl, A<IDictionary<string, string>>._, A<CancellationToken>._)).MustHaveHappenedOnceExactly();
    ...
    public async Task ExecuteAsync_WithActiveCloseToExpiry_NotifiesCreator(int days, ExpiryCheckTypeId expiryCheckTypeId, ExpiryCheckTypeId? currentExpiryCheckTypeId)
    {
        ...
        A.CallTo(() => _companySsiDetailsRepository.RemoveSsiDetail(ssiDetail.Id)).MustNotHaveHappened();
        A.CallTo(() => _issuerRepositories.SaveAsync()).MustHaveHappenedOnceExactly();
        A.CallTo(() => _portalService.AddNotification(A<string>._, Bpnl, NotificationTypeId.CREDENTIAL_EXPIRY, A<CancellationToken>._)).MustHaveHappenedOnceExactly();
        A.CallTo(() => _portalService.TriggerMail("CredentialExpiry", Bpnl, A<IDictionary<string, string>>._, A<CancellationToken>._)).MustHaveHappenedOnceExactly();
