...
#pragma warning restore xUnit1041
    ...
    [Fact]
    public async Task GetAllCredentialDetails_WithBpnl_ReturnsExpected()
    {
        // Arrange
        var sut = await CreateSut();

        // Act
        var result = await sut.GetAllCredentialDetails(null, null, ValidBpnl).ToListAsync();

        // Assert
        result.Should().NotBeNull().And.HaveCount(6);
        ...
        result.Should().HaveCount(6)
            .And.Satisfy(
                ...
                x => x.VerifiedCredentialTypeId == VerifiedCredentialTypeId.PCF_FRAMEWORK,
                x => x.VerifiedCredentialTypeId == VerifiedCredentialTypeId.DISMANTLER_CERTIFICATE,
                x => x.VerifiedCredentialTypeId == VerifiedCredentialTypeId.BEHAVIOR_TWIN_FRAMEWORK,
                x => x.VerifiedCredentialTypeId == VerifiedCredentialTypeId.DISMANTLER_CERTIFICATE,
                x => x.VerifiedCredentialTypeId == VerifiedCredentialTypeId.DISMANTLER_CERTIFICATE);
        result.Should().NotContain(x => x.Bpnl != ValidBpnl);
    }
