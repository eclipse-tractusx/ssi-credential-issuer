...
public class IssuerDbContextTests : IAssemblyFixture<TestDbFixture>
{
    ...
    public async Task SaveCreatedAuditableEntity_SetsLastEditorId()
    {
        ...
        var auditEntries = await sut.AuditCompanySsiDetail20240618.Where(x => x.Id == id).ToListAsync();
        auditEntries.Should().ContainSingle().Which.Should().Match<AuditCompanySsiDetail20240618>(
