...
public class CredentialExpiryProcessHandlerTests
{
    ...
    public async Task RevokeCredential_WithValidData_ReturnsExpected()
    {
        ...
        var credential = new CompanySsiDetail(_credentialId, "Test", VerifiedCredentialTypeId.BUSINESS_PARTNER_NUMBER, CompanySsiDetailStatusId.ACTIVE, "Test123", Guid.NewGuid(), DateTimeOffset.UtcNow);
    ...
    public async Task TriggerNotification_WithValid_CallsExpected()
    {
        ...
        A.CallTo(() => _credentialRepository.GetCredentialNotificationData(_credentialId))
            .Returns(new ValueTuple<VerifiedCredentialTypeId, Guid>(VerifiedCredentialTypeId.PCF_FRAMEWORK, requesterId));
