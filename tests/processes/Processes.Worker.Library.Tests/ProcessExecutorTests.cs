...
public class ProcessExecutorTests
{
    ...
    public ProcessExecutorTests()
    {
        ...
        A.CallTo(() => issuerRepositories.GetInstance<IProcessStepRepository>())
            .Returns(_processStepRepository);

        A.CallTo(() => _processTypeExecutor.GetProcessTypeId())
            .Returns(ProcessTypeId.CREATE_CREDENTIAL);
    ...
    public void GetRegisteredProcessTypeIds_ReturnsExpected()
    {
        ...
        // Assert
        result.Should().HaveCount(1).And.Contain(ProcessTypeId.CREATE_CREDENTIAL);
    }
    ...
    {
        // Arrange
        IAsyncEnumerable<IProcessExecutor.ProcessExecutionResult> Act() => _sut.ExecuteProcess(Guid.NewGuid(), default, CancellationToken.None);

        // Act
        var result = await Assert.ThrowsAsync<UnexpectedConditionException>(async () => await Act().ToListAsync());
