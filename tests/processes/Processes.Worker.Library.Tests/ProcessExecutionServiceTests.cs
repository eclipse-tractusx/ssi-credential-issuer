...
public class ProcessExecutionServiceTests
{
    ...
    public async Task ExecuteAsync_WithNoPendingItems_NoServiceCall()
    {
        ...
        // Assert
        A.CallTo((Expression<Action>)(() => _processExecutor.ExecuteProcess(A<Guid>._, A<ProcessTypeId>._, A<CancellationToken>._)))
            .MustNotHaveHappened();
    }
    ...
    {
        // Arrange
        var processData = _fixture.CreateMany<Guid>().Select(x => new Process(x, ProcessTypeId.CREATE_CREDENTIAL, Guid.NewGuid())).ToImmutableArray();
        A.CallTo(() => _processStepRepository.GetActiveProcesses(A<IEnumerable<ProcessTypeId>>._, A<IEnumerable<ProcessStepTypeId>>._, A<DateTimeOffset>._))
        ...
        A.CallTo(() => _issuerRepositories.SaveAsync())
            .MustHaveHappened(processData.Length * 2, Times.Exactly);
        A.CallTo(() => _issuerRepositories.Clear())
            .MustHaveHappened(processData.Length * 2, Times.Exactly);
    }
    ...
    public async Task ExecuteAsync_ExecuteProcess_Returns_RequestLock()
    {
        ...
        var process = new Process(processId, ProcessTypeId.CREATE_CREDENTIAL, processVersion);
    ...
    public async Task ExecuteAsync_ExecuteProcess_Returns_RequestLockThenThrows()
    {
        ...
        var firstProcess = new Process(firstId, ProcessTypeId.CREATE_CREDENTIAL, firstVersion);
        var secondProcess = new Process(secondId, ProcessTypeId.CREATE_CREDENTIAL, secondVersion);
        ...
        // Assert
        A.CallTo((Expression<Action>)(() => _processExecutor.ExecuteProcess(firstId, A<ProcessTypeId>._, A<CancellationToken>._)))
            .MustHaveHappenedOnceExactly();
        A.CallTo((Expression<Action>)(() => _processExecutor.ExecuteProcess(secondId, A<ProcessTypeId>._, A<CancellationToken>._)))
            .MustHaveHappenedOnceExactly();
        A.CallTo(() => _issuerRepositories.SaveAsync())
    ...
    public async Task ExecuteAsync_ExecuteProcess_Returns_UnmodifiedSafeRequested()
    {
        // Arrange
        var firstVersion = Guid.NewGuid();
        var process = new Process(Guid.NewGuid(), ProcessTypeId.CREATE_CREDENTIAL, firstVersion);
        var processData = new[] { process };
        A.CallTo(() => _processStepRepository.GetActiveProcesses(A<IEnumerable<ProcessTypeId>>._, A<IEnumerable<ProcessStepTypeId>>._, A<DateTimeOffset>._))
            .Returns(processData.ToAsyncEnumerable());
    ...
    public async Task ExecuteAsync_ExecuteProcess_Returns_RequestLock_SaveAsyncThrows()
    {
        ...
        var firstVersion = Guid.NewGuid();
        var firstProcess = new Process(firstProcessId, ProcessTypeId.CREATE_CREDENTIAL, firstVersion);
        var secondProcessId = Guid.NewGuid();
        var secondVersion = Guid.NewGuid();
        var secondProcess = new Process(secondProcessId, ProcessTypeId.CREATE_CREDENTIAL, secondVersion);
        var thirdProcessId = Guid.NewGuid();
        var thirdVersion = Guid.NewGuid();
        var thirdProcess = new Process(thirdProcessId, ProcessTypeId.CREATE_CREDENTIAL, thirdVersion);
    ...
    public async Task ExecuteAsync_IgnoresLockedProcesses_LogsInformation()
    {
        ...
        // Arrange
        var processData = _fixture.CreateMany<Guid>().Select(x => new Process(x, ProcessTypeId.CREATE_CREDENTIAL, Guid.NewGuid()) { LockExpiryDate = lockExpiryDate }).ToImmutableArray();
        A.CallTo(() => _processStepRepository.GetActiveProcesses(A<IEnumerable<ProcessTypeId>>._, A<IEnumerable<ProcessStepTypeId>>._, A<DateTimeOffset>._))
            .Returns(processData.ToAsyncEnumerable());
